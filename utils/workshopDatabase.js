...
async function expire(client, workshopId) {
  ...
  await client.query(`update public.workshop_registrations set status = 'expired', reservation_expires_at = null, updated_at = $1 where status in ('pending', 'reserved_cash') and reservation_expires_at <= $1 ${filter}`, args);
}
...
async function metrics(workshopId) {
  ...
  const { rows } = await getPool().query(`select
    ...
    count(*) filter (where status in ('reserved_cash', 'cash_paid') and (status = 'cash_paid' or reservation_expires_at > $2))::int as cash_reserved,
...
async function confirmDeposit({ registrationId, paypalOrderId, paypalCaptureId, transactionId }) { const registration = await byId(registrationId); if (!registration || registration.paymentMethod !== 'cash' || registration.status !== 'reserved_cash') throw new Error('Cash reservation could not be found.'); if (registration.depositStatus === 'paid') return registration; return update({ registrationId, patch: { depositStatus: 'paid', depositAmount: CASH_DEPOSIT_AMOUNT, paypalOrderId, paypalCaptureId, transactionId: transactionId || paypalCaptureId || paypalOrderId } }); }
