...
async function expire(client, workshopId) {
  ...
  await client.query(`update public.workshop_registrations set status = 'cancelled', reservation_expires_at = null, updated_at = $1 where (status = 'pending' or (status = 'reserved_cash' and coalesce(deposit_status, 'pending') <> 'paid')) and reservation_expires_at <= $1 ${filter}`, args);
}
...
async function create(input) {
  ...
  return inTransaction(async (client) => {
    ...
    const values = [registrationId(), code, workshopId, participantName, email, phone, status, paymentMethod, isCash ? 'pending' : null, isCash ? CASH_DEPOSIT_AMOUNT : 0, isCash ? 'pending' : null, input.reservationExpiresAt || ((status === 'pending' || isCash) ? now + DEFAULT_RESERVATION_MINUTES * 60_000 : null), now, now, input.currency || 'EUR', isCash ? CASH_RESERVATION_TOTAL_AMOUNT : Number(input.amount || 0)];
...
async function confirmDeposit({ registrationId, paypalOrderId, paypalCaptureId, transactionId }) { const registration = await byId(registrationId); if (!registration || registration.paymentMethod !== 'cash' || registration.status !== 'reserved_cash') throw new Error('Cash reservation could not be found.'); if (registration.depositStatus === 'paid') return registration; return update({ registrationId, patch: { depositStatus: 'paid', depositAmount: CASH_DEPOSIT_AMOUNT, paymentApprovedAt: Date.now(), reservationExpiresAt: null, paypalOrderId, paypalCaptureId, transactionId: transactionId || paypalCaptureId || paypalOrderId } }); }
